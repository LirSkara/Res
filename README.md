...
Система оснащена многоуровневой защитой от DDoS-атак и вредоносных запросов:

- **⏱️ Ограничение скорости запросов (Rate Limiting):**
  - Отслеживание количества запросов от каждого IP-адреса
  - Автоматическая временная блокировка при превышении лимитов
  - Настраиваемые параметры: количество запросов, временное окно, длительность блокировки

- **🛡️ Фильтрация подозрительных запросов:**
  - Автоматическое обнаружение попыток сканирования уязвимостей
  - Блокировка запросов к несуществующим административным интерфейсам
  - Обнаружение подозрительных User-Agent и паттернов в URL

- **⚙️ Гибкая настройка:**
  - Белые списки доверенных IP-адресов
  - Отключение в режиме разработки
  - Возможность тонкой настройки параметров под нагрузку

Эти меры помогают защитить систему от перегрузки и атак злоумышленников, обеспечивая стабильную работу API.

## �📊 Расширенное логирование API-запросов

Система оснащена продвинутым механизмом логирования всех API-запросов через специальный middleware:

- **📌 Классификация CRUD-операций** — каждый запрос маркируется по типу операции:
...
- **🧑‍💼 Идентификация пользователя** — каждый запрос содержит информацию о том, кто его выполнил

- **📝 Детализация запросов** — логируются все детали запроса:
  - Метод и путь
  - Параметры запроса
...
- **🗂️ Удобное хранение логов:**
  - `logs/api_requests.log` — общий текстовый лог
  - `logs/api_requests_YYYY-MM-DD.json` — детализированные JSON-логи по дням

Это позволяет легко мониторить активность системы, отслеживать потенциальные проблемы и анализировать использование API.
